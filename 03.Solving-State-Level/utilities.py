...
class State(torch_geometric.data.Data):
    ...
    def to(self, device):
        ...
        return State(
            constraint_features=self.constraint_features.to(device) if isinstance(self.constraint_features, torch.Tensor) else self.constraint_features,
            edge_index=self.edge_index.to(device) if isinstance(self.edge_index, torch.Tensor) else self.edge_index,
            edge_attr=self.edge_attr.to(device) if isinstance(self.edge_attr, torch.Tensor) else self.edge_attr,
            variable_features=self.variable_features.to(device) if isinstance(self.variable_features, torch.Tensor) else self.variable_features,
            action_set=self.action_set.to(device) if isinstance(self.action_set, torch.Tensor) else self.action_set,
            action_set_size=self.action_set_size,
    ...
    def __init__(self, constraint_features, edge_indices, edge_features, variable_features,
        ...
        if candidates is not None:
            if isinstance(candidates, torch.Tensor):
                self.nb_candidates = candidates.size(0)
            else:
