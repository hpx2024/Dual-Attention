...
exp_env = int(args.exp_env)    # experiment environment (1: foldable vs unfoldable)
rd.seed(0)
...
def generateMILPunfoldable(k_data, configs, folder):
    ...
    count_feas = 0    # counter for feasible instances
    ...
    for k in range(k_data):
        ...
        # Objective coefficients from standard normal distribution, scaled by 0.01
        c = np.array([rd.normalvariate(0, 1) for j in range(n)]) * 0.01
        # Right-hand side constants for constraints
        b = np.array([rd.normalvariate(0, 1) for i in range(m)])
        # Constraint types: 0 (<=), 1 (==), 2 (>=)
        circ = np.array([rd.randint(0, 2) for i in range(m)])
        # Variable types: 1 for integer, 0 for continuous
        ...
        # Randomly select positions for nonzero elements
        EdgeIndex1D = rd.sample(range(m * n), nnz)
        # Generate edge features from standard normal distribution
        ...
        for i in range(m):
            if circ[i] == 0:
                opt_model.addCons(
                    scip.quicksum
                    (A[i, j] * x_vars[j] for j in range(n)) <= b[i],
        ...
        opt_model.setObjective(
            scip.quicksum(x_vars[j] * c[j] for j in range(n)), "minimize"
        )

        # Save the MILP instance features
        np.savetxt(
            path + '/ConFeatures.csv',
            np.hstack((b.reshape(m, 1), circ.reshape(m, 1))),
            delimiter=',', fmt='%10.5f'
        )
        np.savetxt(
            path + '/EdgeFeatures.csv',
            EdgeFeature, fmt='%10.5f'
        )
        np.savetxt(
            path + '/EdgeIndices.csv',
            EdgeIndex,
            delimiter=',',
            fmt='%d'
        )
        np.savetxt(
            path + '/VarFeatures.csv',
            np.hstack(
                (
                    c.reshape(n, 1),
                    NI.reshape(n, 1),
        ...
        # Solve the MILP using SCIP solver
        opt_model.optimize()

        # Save results based on solution status
...
def generateMILPfoldable(k_data, configs, folder):
    ...
    for k in range(int(k_data / 2)):
        # Generate two related instances per iteration
        k1 = 2 * k
        ...
        # Set objective coefficients based on environment
        if env == 2:
            c = np.ones((n,)) * 0.01
        else:
            c = np.zeros((n,))
        ...
        # Select 6 variables to be binary and form cycles
        ...
        # Create first foldable structure (hexagonal cycle)
        A1 = np.zeros((m, n))
        ...
        # Create second foldable structure (different cycle pattern)
        A2 = np.zeros((m, n))
        ...
        # Add equality constraints to both models
        for i in range(m):
        ...
        # Save results for first instance
        ...
        # Save results for second instance
...
def combineGraphsAll(ID_start, ID_end, configs, folder):
    ...
    def my_stack(ori, aug):
        # Stack arrays, handling None case
    ...
    for k in range(ID_start, ID_end):
        ...
        # Update edge indices with offset to ensure uniqueness across instances
        edgeIndices[:, 0] = edgeIndices[:, 0] + startMIPidx * m
        edgeIndices[:, 1] = edgeIndices[:, 1] + startMIPidx * n

        # Combine current instance data with accumulated data
...
def combineGraphsFeas(ID_start, ID_end, configs, folder):
    ...
    # Collect paths of feasible instances only
    path_list = []
    for k in range(ID_start, ID_end):
        MIP_path = MIP_key + '_' + str(k)
    ...
    # Save combined feasible features and labels
    if not os.path.exists(folder):
        os.mkdir(folder)
...
# Main script
if exp_env == 1:
    # Environment 1: Compare foldable vs unfoldable instances
